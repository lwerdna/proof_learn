...
def load_stdlib():
	assign_macro('RETARG0', r'\x[\y[x]]')
	assign_macro('RETARG1', r'\x[\y[y]]')
	assign_macro('TRUE', r'RETARG0')
	assign_macro('FALSE', r'RETARG1')
	assign_macro('ITE', r'\cond[\a[\b[((cond a) b)]]]')
	assign_macro('K', r'\x[\y[x]]')
	assign_macro('RET_TRUE', r'(K TRUE)')
	assign_macro('RET_FALSE', r'(K FALSE)')
	assign_macro('I', r'\x[x]')
	assign_macro('IF', r'\c[c]')
	assign_macro('OR', r'\x[\y[(((IF x) TRUE) y)]]')
	assign_macro('AND', r'\x[\y[(((IF x) y) FALSE)]]')
	assign_macro('NOT', r'\x[(((IF x) FALSE) TRUE)]')
	assign_macro('0', r'\f[\x[x]]')
	assign_macro('1', r'\f[\x[(f x)]]')
	assign_macro('2', r'\f[\x[(f (f x))]]')
	assign_macro('3', r'\f[\x[(f (f (f x)))]]')
	assign_macro('4', r'\f[\x[(f (f (f (f x))))]]')
	assign_macro('5', r'\f[\x[(f (f (f (f (f x)))))]]')
	assign_macro('6', r'\f[\x[(f (f (f (f (f (f x))))))]]')
	assign_macro('7', r'\f[\x[(f (f (f (f (f (f (f x)))))))]]')
	assign_macro('8', r'\f[\x[(f (f (f (f (f (f (f (f x))))))))]]')
	assign_macro('9', r'\f[\x[(f (f (f (f (f (f (f (f (f x)))))))))]]')
	assign_macro('10', r'\f[\x[(f (f (f (f (f (f (f (f (f (f x))))))))))]]')
	assign_macro('SUCC', r'\n[\f[\x[(f ((n f) x))]]]')
	assign_macro('PLUS', r'\n[\m[\f[\x[((n f) ((m f) x))]]]]')
	assign_macro('MULT', r'\n[\m[\f[\x[((n (m f)) x)]]]]')
	assign_macro('EXP', r'\n[\m[(m n)]]')
	assign_macro('ISZERO', r'\n[((n (K FALSE)) TRUE)]')
	assign_macro('PAIR', r'\a[\b[\p[((p a) b)]]]')
	assign_macro('FIRST', r'\p[(p RETARG0)]')
	assign_macro('SECOND', r'\p[(p RETARG1)]')
	assign_macro('PRED_F', r'\p[((PAIR (SECOND p)) (SUCC (SECOND p)))]')
	assign_macro('PRED', r'\n[(FIRST ((n PRED_F) ((PAIR 0) 0)))]')
	assign_macro('Y', r'\f[(\x[(f (x x))] \x[(f (x x))])]')
